...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        String sqlProgram = "CREATE TABLE Program(program_id INTEGER PRIMARY KEY AUTOINCREMENT, max_step INTEGER NOT NULL, step INTEGER CHECK(step <= max_step), pro_content VARCHAR, FOREIGN KEY(patient_id) REFERENCES Patient(patent_id))";
...
/*        String sqlAsk = "CREATE TABLE Ask(id INTEGER PRIMARY KEY AUTOINCREMENT, date DATE NOT NULL, FOREIGN KEY (question_id) REFERENCES Question(question_id)," +
    ...
    public boolean AddProgram(int max_step, int step, String pro_content, int patient_id ) {
        ...
        contentValues.put("content", pro_content);
...
*/
    ...
    public Program getProgram(long patient_id) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        String selectQuery = "SELECT * FROM Program WHERE patient_id = " + patient_id + "AND step = 1 LIMIT 1";
        ...
        for (int i = 1; i<=max_step; ++i) {
            ...
            if (c != null)
                c.moveToFirst();
            program.setPro_content("Step "+ i+ " : " +c.getString(c.getColumnIndex("pro_content")));
        }
