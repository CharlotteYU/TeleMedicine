...
import com.example.navigater.model.Medicine;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        String sqlMedicine = "CREATE TABLE Medicine(medicine_id INTEGER PRIMARY KEY AUTOINCREMENT, medicine_name VARCHAR NOT NULL,medicine_nb INTEGER, patientm_id INTEGER, FOREIGN KEY(patientm_id) REFERENCES Patient(patent_id))";
...
/*        String sqlAsk = "CREATE TABLE Ask(id INTEGER PRIMARY KEY AUTOINCREMENT, date DATE NOT NULL, FOREIGN KEY (question_id) REFERENCES Question(question_id)," +
    ...
    public boolean AddMedicine(String medicine_name, int medicine_nb, int patientm_id ) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("medicine_name", medicine_name);
        contentValues.put("medicine_nb", medicine_nb);
        contentValues.put("patientm_id", patientm_id);
...
*/
    ...
    public Medicine getMedicine(long patient_id) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        String selectQuery = "SELECT COUNT(DISTINCT medicine_name) AS max FROM Medicine WHERE patientm_id = " + patient_id;

        Cursor c = sqLiteDatabase.rawQuery(selectQuery, null);
        if (c != null)
            c.moveToFirst();
        int me_nb = c.getInt(c.getColumnIndex("max"));

        Medicine medicine = new Medicine();
        medicine.setNb(me_nb);

        String selectQuery_m = "SELECT DISTINCT medicine_name,medicine_nb FROM Medicine WHERE patientm_id = " + patient_id;
        Cursor cc = sqLiteDatabase.rawQuery(selectQuery_m, null);
        if (cc != null) {
            cc.moveToFirst();
            medicine.setMedicine(cc.getString(cc.getColumnIndex("medicine_name")) + " : " +cc.getInt(cc.getColumnIndex("medicine_nb")));
            while (cc.moveToNext()) {
                medicine.setMedicine(cc.getString(cc.getColumnIndex("medicine_name")) + " : " +cc.getInt(cc.getColumnIndex("medicine_nb")));
            }
        }
